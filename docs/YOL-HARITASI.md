...
| A3 | Gerçek PostgreSQL + tarayıcı çalıştıran CI | **kanıt bekliyor** | `.github/workflows/ci.yml` yazıldı: Node 24.21.0 + 22.22.2, `postgres:16` servisi, pinli Chromium, veritabanısız ve izole PostgreSQL koşuları **ayrı adımlar**. **Hiç çalışmadı** — Actions bu depoda hiç koşmadı, bu yüzden buradan çıkan hiçbir sonuç kanıt sayılamaz. İlk koşuyu açmak Hasan'da |
